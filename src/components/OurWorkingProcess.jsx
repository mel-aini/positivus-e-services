...
function Process({index, title, paragraph, mainColor, grayColor}) {
    const [expand, setExpand] = useState(false);
    const [sign, setSign] = useState('+');
    const [heightExpand, setHeightExpand] = useState('115px');
    const [bgColor, setBgColor] = useState(grayColor);
    ...
    return (
        <div className="process w-full shadow-[0_5px] shadow-black rounded-[45px] py-10 px-8 duration-300 ease-out overflow-hidden border-[1px] border-black border-solid" style={{backgroundColor: bgColor, maxHeight: heightExpand, zIndex: index}}>
            <div className="flex justify-between items-center relative">
                ...
                }} className="expand-btn cursor-pointer font-bold min-w-[35px] min-h-[35px] rounded-[50%] border-2 border-solid border-black flex justify-center items-center" style={ {backgroundColor: grayColor }}>{sign}</span>
            </div>
...
function OurWorkingProcess({ mainColor, grayColor }) {
    const processes = [
        ...
        <div className="w-[90%] mx-auto my-5">
            <div className="w-full flex flex-col gap-[40px] lg:flex-row justify-center lg:justify-start items-center py-[50px]">
                <div className="flex flex-col lg:flex-row items-center lg:items-start font-bold text-5xl lg:rounded-xl lg:bg-[#B9FF66]">
                    <h1 className="py-2 px-3 rounded-xl" style={{backgroundColor: mainColor}}>Our Working</h1>
                    <h1 className="py-2 px-3 rounded-xl w-fit" style={{backgroundColor: mainColor}}>Process</h1>
                </div>
            ...
            <div className="flex flex-col gap-5">
                {processes.map((elem) => {
                    return (
                        <Process index={elem.index} title={elem.title} paragraph={elem.paragraph} key={elem.index} mainColor={mainColor} grayColor={grayColor}/>
                    )
                })}
            </div>
        </div>
